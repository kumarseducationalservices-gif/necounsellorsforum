    ...
    <div className="space-y-2.5 py-3">
      <div className="flex flex-wrap gap-2">
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          {SORT.map(([v,l]) => (
            <button key={v} style={pill(!params.sort && v==='' || params.sort===v,'#00875A')} onClick={() => update('sort',v)}>{l}</button>
          ))}
        </div>
      </div>
      <div className="flex flex-wrap gap-2">
        <span className="text-xs py-1 px-1" style={{ color:'var(--muted)' }}>City:</span>
        {NE_CITIES.map(c => <button key={c} style={pill(params.city===c,'#FF5A1F')} onClick={() => update('city', params.city===c?'':c)}>{c}</button>)}
      </div>
      <div className="flex flex-wrap gap-2">
        <span className="text-xs py-1 px-1" style={{ color:'var(--muted)' }}>Topic:</span>
        {SPECS.map(s => <button key={s} style={pill(params.specialization===s,'#D4AF37')} onClick={() => update('specialization', params.specialization===s?'':s)}>{s}</button>)}
      </div>
    </div>
  )
}
